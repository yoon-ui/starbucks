$(function(){
   ...
   // var offerOption={
   //     //숫자를 넣으면 넣은 만큼의 슬라이드 수를 보여주며 
   //     //auto로 처리하면 슬라이드 하나당의 넓이를 내가 지정하여 처리
   //    slidesPerView:'auto',
   //    //spaceBwtween 은 margin이라고 생각하면 됨다
   //    spaceBetween:20,
   //    //center옵션을 넣었을 때 loop를 같이 넣어주면 이전슬라이드가 
   //    //복제되서 왼쪽편에 나타난다.
   //    loop:true, 
   //    //현재 보고있는 슬라이드 페이지가 중심에 보이도록 처리
   //    centeredSlides:true     
   // }
   ...
   // //매장검색시 나오는 리스트 목록 클릭시 활성화 상태 변경
   // $('#store list').on('click',' a',function(e){
   //    //href의 기능 사용하지 않기
   //    e.preventDefault()
   //    $('#store list li').removeClass(className);
